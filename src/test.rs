use crate::sim::*;
use crate::parser::*;
use std::{fs,fs::File, io::BufReader, collections::HashMap, path::Path};
...
fn check_solution(stats: &mut(usize, usize), fpath:&Path, puzzle_map: &PuzzleMap){
    ...
    let sol = match sol_maybe{
        Err(e) => {println!("Failed to parse solution {:?}: {}",fpath, e); return;}
        Ok(s) => s
    };
    let puzzle = puzzle_map.get(&sol.puzzle_name);
    if puzzle.is_none(){
        /*println!("Can't find puzzle {}",sol.puzzle_name);*/ return;
    }
    if let Some(puzzle) = puzzle{
        let init = match puzzle_prep(&puzzle, &sol){
            Err(e) => {println!("Failed to prepare {:?}: {}",fpath, e); return}
            Ok(s) => s
        };
        let mut world = match World::setup_sim(&init){
            Err(e) => {println!("Failed to setup {:?}: {}",fpath, e); return}
            Ok(s) => s
        };
        stats.1 += 1;
...
fn check_all(){
    ...
    fn read_solution_recurse(stats: &mut(usize, usize), puzzle_map: &PuzzleMap, directory: fs::ReadDir){
        for f in directory{
            if let Ok(f) = f{
                let ftype = f.file_type().unwrap();
                if ftype.is_dir(){
